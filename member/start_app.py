...
def delete(bno):
    # 삭제 요청한 글 번호를 DB board 테이블에서 삭제
    conn = getconn()
    cursor = conn.cursor()
    sql = f"DELETE FROM board WHERE bno = {bno}" #숫자이믈 따옴표 붙이지 않음
    cursor.execute(sql)
    conn.commit()
    conn.close()
    return redirect(url_for('boardlist'))
...
def update(bno):
    if request.method == "POST":
        ...
        return redirect(url_for('detail', bno=bno)) # .html 빼기 / 상세보기(글번호 명시!)
    else:
        ...
        sql = f"SELECT * FROM board WHERE bno = {bno}"
        cursor.execute(sql)
        board = cursor.fetchone()   #게시글 1개 반환방음
        conn.close()
