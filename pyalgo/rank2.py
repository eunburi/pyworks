...
rank = [0, 0, 0, 0, 0, 0,0] #리스트 요소를 0으로 초기화
n = len(score)
print(n)
...
for i in range(0, n):
    ...
    for j in range(0, n):
        if score[i] < score[j]:
            count += 1
    rank[i] = count #변경된 count를 rank리스트에 저장
print(rank)
...
i=0 j=0 score[0] < score[0] False rank[0]=1
    ...
    j=5 score[0] < score[4] False rank[0]=2(순위확정)
...
i=1 j=0 score[1] < score[0] True rank[1]=2   # count = 2
    ...
    j=4 score[1] < score[4] True rank[1]=4  # count = 5
    j=5 score[1] < score[5] True rank[1]=5 (순위확정)

      
i=2 j=0 score[2] < score[0] True rank[2]=2
    j=1 score[2] < score[1] False rank[2]=2
    j=2 score[2] < score[2] False rank[2]=2 (순위확정)
    j=3 score[2] < score[3] False rank[2]=2
    j=4 score[2] < score[4] False rank[2]=3
    j=5 score[2] < score[5] False rank[2]=3
